...
**Đang redesign GameHub** (giao diện đẹp/chuyên nghiệp hơn, kiểu itch.io theo style CTS Lab) — chia 3 sub-project: **A ✅** mở rộng data model + **trang chi tiết game redesign** (hero, badge, mô tả Markdown, sidebar). **B ✅** redesign **trang hub** (header + search + filter pill tự ẩn + GameCard đẹp + empty state). **C ⏸ (tạm dừng, làm sau)** Creator Studio: dashboard "game của tôi" + form tạo/sửa nhiều section + route update.
